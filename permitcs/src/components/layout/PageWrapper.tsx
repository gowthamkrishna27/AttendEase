...
import {
  Home, FileText, Clock, User, LogOut,
  Bell, GraduationCap, Menu, Plus,
} from 'lucide-react';
...
const studentNav = [
  ...
  { to: '/student/new-request', label: 'My Requests', icon: FileText },
  { to: '/student/history', label: 'History', icon: Clock },
  { to: '/student/profile', label: 'Profile', icon: User },
];
const facultyNav = [{ to: '/faculty', label: 'Dashboard', icon: Home }];
const hodNav = [{ to: '/hod', label: 'Overview', icon: Home }];

/* Bottom tab bar items (mobile) — center + button is separate */
const mobileBottomNav = [
  { to: '/student',             label: 'Home',     icon: Home     },
  { to: '/student/new-request', label: 'Requests', icon: FileText },
  null, // placeholder for centre + button
  { to: '/student/history',     label: 'History',  icon: Clock    },
  { to: '/student/profile',     label: 'Profile',  icon: User     },
];
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message: 'Your attendance request for "Medical Leave" (14 Jul) has been approved by Faculty.',
      time: '8 days ago',
      unread: true,
    },
    {
      ...
      message: 'Your attendance request for "Sports Event" (12 Jul) has been rejected by Faculty.',
      time: '10 days ago',
      unread: true,
    },
    {
      ...
      message: 'Your attendance request for "Workshop" (17 Jul) has been successfully submitted and is pending review.',
      time: '4 days ago',
      unread: true,
    },
  ]);
  ...
  const renderNotificationDropdown = () => {
    ...
    return (
      <div style={{
        ...
        borderRadius: 16, boxShadow: '0 10px 30px rgba(13,27,42,0.12)',
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#F3F6FB', fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
      ...
      <header className="mobile-topbar" style={{
        ...
        <div style={{ marginLeft: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
          <div ref={mobileBellRef} style={{ position: 'relative' }}>
            <button
              onClick={() => setShowNotifications(!showNotifications)}
              style={{ width: 36, height: 36, borderRadius: 10, background: '#F8FAFC', border: '1px solid #E8EDF2', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#64748B' }}
            >
              <Bell size={17} />
            </button>
            {unreadCount > 0 && (
              <span style={{ position: 'absolute', top: 2, right: 2, width: 8, height: 8, borderRadius: '50%', background: '#F97316', border: '1.5px solid #fff' }} />
            )}
            {renderNotificationDropdown()}
          </div>
          <button style={{ width: 36, height: 36, borderRadius: 10, background: '#F8FAFC', border: '1px solid #E8EDF2', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#64748B' }}>
            <Menu size={18} />
          </button>
        </div>
      </header>
