...
export default function FacultyDashboard() {
  ...
  const filtered = requests.filter((r: AttendanceRequest) => {
    const matchesSearch =
  ...
  const counts = {
    ...
    pending: requests.filter((r: AttendanceRequest) => r.status === 'pending').length,
    approved: requests.filter((r: AttendanceRequest) => r.status === 'approved').length,
    rejected: requests.filter((r: AttendanceRequest) => r.status === 'rejected').length,
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          <div className="hidden sm:flex items-center gap-3">
            <Avatar name={user?.name ?? 'F'} size="md" role="faculty" />
            <div>
              <p className="text-[14px] font-semibold text-slate-800">{user?.name}</p>
              <p className="text-[12px] text-slate-400">{user?.department}</p>
            </div>
          </div>
        </motion.div>
            ...
            {filtered.map(req => (
              ...
              >
                ...
                <div className="flex items-center gap-2 ml-3 flex-shrink-0">
                  ...
                  {req.status === 'pending' && (
                    <div
                      className={`flex items-center gap-1.5 transition-all duration-150 ${hoveredId === req.id ? 'opacity-100' : 'opacity-0'
                        }`}
                    >
                      <button
