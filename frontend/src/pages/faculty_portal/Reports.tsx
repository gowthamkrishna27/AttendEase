...
import {
  ClipboardList, Clock, CheckCircle2, XCircle, BarChart2,
...
export default function FacultyReports() {
  ...
  const downloadExcel = (statusFilter: string = 'all') => {
    ...
    XLSX.writeFile(wb, `AttendEase_Faculty_Report_${statusFilter.toUpperCase()}_${dateStr}.xlsx`);
  };
    ...
    <PageWrapper role="faculty" showGreeting={false}>
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <p className="text-[14px] text-slate-400 mt-1">Attendance permission analytics &amp; live Excel spreadsheet reports</p>
        </motion.div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-2.5 w-full sm:w-auto flex-wrap">
              ...
              <button
                ...
                title="Download assigned records as .xlsx"
              >
                <Download size={15} />
        ...
        >
          {[
            { icon: ClipboardList, label: 'Total Requests', value: total,    sub: 'All time'                  },
            { icon: Clock,         label: 'Pending',         value: pending,  sub: `${pendingRate}% of total`  },
            { icon: CheckCircle2,  label: 'Approved',        value: approved, sub: `${approvalRate}% approval`},
            { icon: XCircle,       label: 'Rejected',        value: rejected, sub: `${rejectedRate}% of total` },
          ].map(s => {
            const Icon = s.icon;
            return (
              <div key={s.label} className="card px-4 py-4 flex flex-col gap-2">
                ...
                <div>
                  <p className="text-[24px] font-heading font-bold text-slate-900">{s.value}</p>
      ...
      <ExcelSheetViewerModal
        ...
        title="Faculty Attendance Permission Spreadsheet"
        role="faculty"
      />
    </PageWrapper>
  );
