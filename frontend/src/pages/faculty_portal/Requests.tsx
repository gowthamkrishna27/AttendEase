...
export default function FacultyRequests() {
  ...
  const reviewMutation = useMutation({
    ...
    onMutate: async ({ id, action }) => {
      ...
      const previous = queryClient.getQueryData<AttendanceRequest[]>(['requests']);
      if (previous) {
        queryClient.setQueryData<AttendanceRequest[]>(['requests'], old =>
          (old || []).map(r => r.id === id ? { ...r, status: action === 'approve' ? 'approved' : 'rejected' } : r)
        );
      }
      return { previous };
    },
  ...
  const handleReject = (id: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    const reason = window.prompt('Enter rejection reason (optional):') || 'Rejected by Faculty';
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between pt-0.5" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <Check size={12} strokeWidth={2.5} /><span>Approve</span>
                          </button>
                          <button
                            onClick={e => handleReject(req.id, e)}
                          ...
                          >
                            <X size={13} strokeWidth={2.5} /><span>Reject</span>
                          </button>
                        </div>
                      </div>
                    </motion.div>
