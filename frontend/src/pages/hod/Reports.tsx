...
export default function HODReports() {
  ...
  const filteredRequests = useMemo(() => {
    return requestsList.filter(r => {
      ...
      const faculty = (r.faculty?.name || r.facultyName || '').toLowerCase();
  ...
  const downloadGroupedExcel = (specificReasonKey?: string) => {
    ...
    // Summary Sheet
    const summaryRows = [
      { Metric: 'Total Permission Requests', Count: total, Rate: '100%' },
    ...
    targetGroups.forEach(group => {
      const sheetName = group.label.slice(0, 31).replace(/[/\\?*:[\]]/g, ''); // Sheet names max 31 chars
      const rows = group.requests.map((r, idx) => ({
        ...
        'Reviewer Faculty': r.faculty?.name || r.facultyName || '—',
    ...
    const filename = specificReasonKey
      ? `AttendEase_Report_${specificReasonKey}_${dateStr}.xlsx`
      : `AttendEase_Grouped_Reason_Report_${dateStr}.xlsx`;
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-medium text-slate-400">Department Analytics</span>
          </div>
          <h1 className="text-[24px] font-bold text-slate-900 tracking-tight mt-1">Attendance Reports by Reason</h1>
          <p className="text-[13px] text-slate-500">Grouped analysis and request ledger organized by student reason categories</p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-2xs">
            ...
            <p className="text-[10.5px] text-slate-400 mt-0.5">{groupedByReason.length} reason categories</p>
          </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-2xs">
            ...
            <p className="text-[10.5px] text-emerald-600 font-semibold mt-0.5">{approvalRate}% approval rate</p>
          </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-2xs">
            ...
            <p className="text-[10.5px] text-amber-600 font-semibold mt-0.5">{pendingRate}% backlog</p>
          </div>
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-4 space-y-3 shadow-2xs">
          ...
          {/* Quick Category Pills & Expand / Collapse Controls */}
                            ...
                            return (
                              <tr key={r.id || idx} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="py-3 px-4 text-slate-600">
                                  {r.faculties && r.faculties.length > 1 ? (
                                    <span
                                      className="text-[11px] font-semibold text-slate-800 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-300"
                                      title={r.faculties.map((f: any) => f.name).join(', ')}
                                    >
                                      Multiple ({r.faculties.length})
                                    </span>
                                  ) : (
                                    <p className="font-medium text-slate-800">
                                      {r.faculties && r.faculties.length === 1
                                        ? r.faculties[0].name
                                        : r.primaryFaculty?.name || r.faculty?.name || r.facultyName || 'Primary Faculty'}
                                    </p>
                                  )}
                                </td>
